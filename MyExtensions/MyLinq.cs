...
public static class MyLinq
{
    ...
    public static IEnumerable<T> Where<T>(this IEnumerable<T>? collection, Predicate<T> cond)
    {
        ...
        foreach (var i in enumerable)
        {
            if (i != null && cond.Invoke(i)) temp.AddLast(i);
        }

        return temp;
    }

    public static bool Any<T>(this IEnumerable<T>? collection, Predicate<T>? cond = null)
    {
        ...
        foreach (var i in enumerable)
        {
            if (i != null && (cond?.Invoke(i) ?? true)) return true;
        }
